	...
	def showWindow(self, sender):
		""" Do something like show a window"""
 		try:
 			...
 			reload(CM)
 			cm = CM.Common()
 			values = cm.getLicense()
 			mode = values[0]
 			daysRemain = values[1]

 			if (0 == mode and 0 <= daysRemain) or (1 == mode):
		 		import CollectHangulModule as CH
		 		reload(CH)
	 			w = CH.Collect(self.numWindows, mode, daysRemain)
	 			w.run()
	 		else:
	 			Message(u'❓ 오류 ❓', u'데모 기간이 만료되었거나 인증 정보가 없습니다.\n등록 후 사용 가능합니다!')
		 		import CollectHangulModule as CH
		 		reload(CH)
		 		strChallenge = cm.genChallenge()
